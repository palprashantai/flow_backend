...
export async function getFilteredPortfolios(filterDto: FilterPortfolioDto, userId?: number) {
  try {
    ...
    const baseSelect = [
      ...
      's.access_price_monthly AS access_price_monthly',
      's.discount_price_monthly AS discount_price_monthly',
      's.service_banner AS service_banner',
    ...
    const portfolios = rows.map((item) => {
      ...
      return {
        ...
        discount_price_monthly: item.discount_price_monthly,
