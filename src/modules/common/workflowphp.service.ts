...
export class WorkflowService {
  ...
  async callSubscriberInsertWorkflow(
    subscriberId: number,
    productApp?: 0 | 1,
  ): Promise<any> {
  ...
  async callSubscriptionInsertWorkflow(
    subscriptionId: number,
    productApp?: 0 | 1,
  ): Promise<any> {
  ...
  async callLeadCreationWorkflow(
    leadId: number,
    productApp?: 0 | 1,
  ): Promise<any> {
  ...
  async callMultipleWorkflows(
    ...
    const promises = calls.map((call) =>
      this.callWorkflow(call.type, call.id, call.productApp ?? 0, call.data).catch(
        (error) => ({
          error: error.message,
          type: call.type,
          id: call.id,
